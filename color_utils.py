...
def compute_inverse_squared_difference_matrix(image, target_color):
    """
    ...
    """
    squared_diff_matrix = np.sum((image - target_color) ** 2, axis=2)
    inverse_squared_diff_matrix = 1 / (1 + squared_diff_matrix)
    return inverse_squared_diff_matrix


def compute_average_isd(image, target_color, mask=None):
    # difference matrix with the euclidean distances
    diff_matrix = compute_inverse_squared_difference_matrix(image, target_color)
...
def find_complementary_color(image, color_dict, mask=None):
    """
    ...
    for color_name, color_rgb in color_dict.items():
        avg_isd = compute_average_isd(image, color_rgb, mask)
